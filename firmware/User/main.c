...
#define CMD_ERASE    0x45  // 'E'  erase storage area
#define CMD_DATA     0x44  // 'D'  page data
#define CMD_FINISH   0x46  // 'F'  finish & set ready flag
#define CMD_INFO     0x49  // 'I'  query partition info

#define MAX_PAYLOAD  1028  // 2B page_index + 1024B page + 2B margin
...
static uint8_t flash_write_buf(uint32_t addr, uint8_t *data, uint16_t len)
{
    FLASH_Status s;
...
static uint8_t storage_erase_all(void)
{
    uint8_t i;
...
static uint8_t storage_write_page(uint8_t idx, uint8_t *data)
{
    uint32_t addr;
...
static uint8_t storage_set_ready_flag(void)
{
...
static void frame_dispatch(void)
{
    ...
    // validate CRC16 over [cmd + len_l + len_h + data]
    ...
    case CMD_DATA:
        // payload: [page_idx:2][page_data:1024]
